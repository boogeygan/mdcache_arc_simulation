...
static void __arc_hash_insert(struct __arc *cache, unsigned int *key, struct __arc_object *obj)
{
	assert((*key) >=  0);
...
static struct __arc_object *__arc_hash_lookup(struct __arc *cache, unsigned int *key)
{
	struct __arc_list *iter;
	assert((*key) >=  0);
	unsigned int hash = cache->ops->hash(key) % cache->hash.size;

	__arc_list_each(iter, &cache->hash.bucket[hash]) {
		struct __arc_object *obj = __arc_list_entry(iter, struct __arc_object, hash);
		if (cache->ops->cmp(obj, key) == 1)
			return obj;
	}
...
{
    cache = malloc(sizeof(struct __arc));
...
void arc_end()
{
	printf("total=%d hits=%d\n",cache->total,cache->hits);
	__arc_hash_fini(cache);
	free(cache);
}

/* Lookup an object with the given key. */
struct __arc_object *__arc_lookup(struct __arc *cache, unsigned int *key, int stat_update )
{
